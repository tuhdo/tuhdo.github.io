...
- A nice feature of =helm-semantic-or-imenu= is that whenever you
  activate the command, and if point is inside a type of Semantic
  tags, the highlighter pre-selects that tag in the Helm buffer. This,
  in combination with *C-<down>* and *C-<up>* to move between
