...
this guide to try out Helm and skip the rest of the installation.
...
- I only want to have buffers that contains only the string
  ...
  matching buffers to =helm-moccur= - =moccur= with Helm 
  interface using *C-s*. Candidates can be filtered gradually by
  adding more pattern, i.e. I added memory to filtered down to buffers
  that contain the string "memory" among the buffers that are
  containing =crash=.
