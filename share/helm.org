...
Search for buffers that contain string "Answer", and transfer the
search patterns to =helm-moccur= for the exact matches (the demo
...
If you don't have =surfraw= program or don't want to use, you can
override *C-c h s* binding with this command.


_Key binding_:

*<prefix> C-c g*, <prefix> is *C-x c* by default, or *C-c h* if
set. If you already use =helm-surfraw=, then this command is not
needed. However, if you want to use, bind it to a key:
