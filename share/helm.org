...
#+begin_src emacs-lisp
  ...
  (require 'helm)
  (require 'helm-config)
  (setq helm-command-prefix-key "C-c h") ; must set before helm-config,
                                          ; otherwise helm use default
                                          ; prefix "C-x c"
  (require 'helm-eshell)
  (require 'helm-files)
  (require 'helm-grep)
...
:END:
_Key binding_: No key binding. We should give it one:
...
in the buffers,  *C-s* while in =helm-mini= session to switch to
=helm-moccur=. By default, =helm-mini= searches all opening buffers; 
you can mark buffers to search by *C-SPC*; note that even if you mark
buffers with *C-SPC*, content search using =@= still lists all buffers
that contain the pattern; only when you switch to =helm-moccur=,
matches that are in selected buffers are displayed. You can also
perform =occur= only on the current buffer with prefix argument: =C-u
C-s=.
...
:END:
_Key binding_: *<prefix> C-x C-f* or *C-x C-f*; =<prefix>= is *C-x c* by
default, or *C-c h* if set. This is a rather long key sequence, and
...
:END:
_Key binding_: *<prefix> i*, =<prefix>= is *C-x c* by default, or *C-c
h* if set.
...
:END:
_Key binding_: *<prefix> m*, =<prefix>= is *C-x c* by default, or *C-c
h* if set.
...
:END:
_Key binding_: *<prefix> /*, =<prefix>= is *C-x c* by default, or *C-c
h* if set. 
...
:END:
_Key binding_: *<prefix> l*, =<prefix>= is *C-x c* by default, or *C-c
h* if set.
...
:END:
_Key binding_: *<prefix> M-s o*, =<prefix>= is *C-x c* by default, or *C-c
h* if set. Since this is a rather long binding, we should bind a more
...
:END:
_Key binding_: *<prefix> a*, =<prefix>= is *C-x c* by default, or *C-c
h* if set.
...
:END:
_Key binding_: *<prefix> h <key>*, =<prefix>= is *C-x c* by default, or *C-c
h* if set; *<key>*, by defaults, is one of *g*, *i* or *r*:
...
:END:
_Key binding_: *<prefix> b*, =<prefix>= is *C-x c* by default, or *C-c
h* if set.
...
:END:
_Key binding_: *<prefix> C-c SPC*, =<prefix>= is *C-x c* by default, or *C-c
h* if set. This is a rather long key sequence, this command deserves a
