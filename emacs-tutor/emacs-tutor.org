...
My personal tour - *with added 3rd party plugins* - from Emacs. Don't
worry if your Emacs does not look like the screenshots. You can have
something like mine and even better, but for now you need to get a
solid foundation. The screenshots are just for show.
...
file, or for displaying text in Emacs if not. If you want to close a
buffer. *C-x k* to kill the buffer.
...
Practice *C-x b* to get used to it.
