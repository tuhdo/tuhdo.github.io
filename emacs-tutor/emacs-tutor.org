...
- Programming:
  ...
  - GDB Many Windows:
    ...
    [[file:static/gdb-many-windows2.png][file:~/org-data/emacs-tutor/static/gdb-many-windows2.png]]
    #+END_CENTER
...
:END:
The current version is Emacs 23, with the upcoming Emacs 24 is on the
...
:PROPERTIES:
:ID:       de0509ca-e6de-4a0a-92ef-b7f18a492252
:END:
You will learn quite a bit about Emacs in this section, enough for
following the demo.

In Emacs, =M-= means =Meta=, which is the =Alt= key and =C-= means the
=Control= key in regular computer keyboard. From now on, when I say
=M-x command=, it means you press =M-x= and type =command= in.

When I say *C-x b*, it means you hold *Control* and *x* keys together,
then release and press *b* key. *C-x b* allows to switch between Emacs
buffers. Buffer is where you edit your file if it is associated with a
file, or for displaying text in Emacs if not.

Emacs Lisp is the language for you to customize and extends
Emacs. Emacs Lisp belongs the the Lisp family of languages. Lisp has a
long history since 1959. At this stage, you don't need to know about
Emacs Lisp.

=*sratch*= buffer is a buffer where you experiment with Emacs Lisp
code.

In Emacs, to copy, you can highlight the text with mouse and *M-w*; to
paste, *C-y*. You can also paste text from external programs, too.

If your Emacs screen is dividied into smaller screens like this:

[[file:static/multiple-windows.gif][file:static/multiple-windows.gif]]

Do you notice the blinking cursor on the top left screen? If you press
*C-x 1*, Emacs will bury other two screens and make the screen with
blinking cursor a whole.

During the demo, if you press weird key sequence and got stuck, press
*C-g* to quit the partially typed key sequence. You will see "Quit"
message at the bottom of your Emacs.
...
- Now, you see a lot of names in this file: variable names, function
  ...
  *M-Shift-8*) brings you back to the previous place where you
  ...
  power of Emacs even further, you need to learn Emacs properly.
...
types of functions in Emacs:
    ...
    Emacs. Users do not need to care about these functions, unless they
    want to change the implementations.
...
*C-g* executes the command =keyboard-quit=, which cancels anything
Emacs is executing. =M-x keyboard-quit=, you will see
this command there. 
...
To save a buffer as other file ("Save As" in other editors), execute
*C-x C-w*.
...
configuration you have been used in =*scratch*= buffer. If you tried
the Linux kernel browsing demo, you have these configurations:
...
** Save file locations across Emacs sessions
:PROPERTIES:
...
same, with differnt but similar key bindings.
...
You can select the old input to search again.
...
windows: two above and one below. So, what can you do the windows you
created? As I said earlier, you can put anything there.
...
| Key       | Binding                                                             |
|-----------+---------------------------------------------------------------------|
| =C-x v == | Displays a diff showing the changes you've made to the current      |
|           | files.                                                              |
|-----------+---------------------------------------------------------------------|
| =C-x v ~= | Prompts you for a version number and shows you that version of the  |
...
|-----------+---------------------------------------------------------------------|
| =C-x v l= | Displays a log of previous changes to the file. When point is       |
...
|-----------+---------------------------------------------------------------------|
| =C-x v u= | Revert working copies of the selected fileset to their repository   |
...
of Emacs and 3rd party packages.
