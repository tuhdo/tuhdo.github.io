...
If you are a programmer, you should automate thing as much as
possible. One way to do this is using a good editor that automates
...
- Save a few buffer positions into registers.
- Jump to the positions in registers.
...
Suppose that you practice Agile and have to write user stories with a
template like this:
...
last names*, so that *I can find the customers I'm needing*.
#+END_QUOTE

It would be tiresome to type the template texts, such as =As a=, =*
want= or =so that=. We should automate the inserting those texts
because it is repetitive. This can be done by just manually typing in
relevant information in a table format:

- Row contains an instance of the text template.
- Column separates types of data.

For example, I have this table with some user stories:

user, to search for my customers by their first and last names, I can find the customers I'm needing
admin, 
...
|               | Insert the counter into the buffer and increase             |
|               | the counter by 1                                            |
|---------------+-------------------------------------------------------------|
...
|---------------+----------------------------------------------------------------------------------------|
| =%x=          | Replace the specification with the base-sixteen representation of an integer.          |
...
|---------------+----------------------------------------------------------------------------------------|
| =%%=          | Replace the specification with a single ‘%’. This format specification is unusual in   |
|               | in that it does not use a value. For example, (format "%% %d" 30) returns "% 30".      |
|---------------+----------------------------------------------------------------------------------------|
...
:PROPERTIES:
:ID:       9ecc6b05-284e-48a3-b780-2b918ab63ee0
:END:
** Version Control
:PROPERTIES:
