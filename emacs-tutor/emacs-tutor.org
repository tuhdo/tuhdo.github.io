...
:END:
When I first heard registers in Emacs, it scared me. I thought "Wow,
what is this "register" thingie? Am I going to work directly with CPU
registers? As it turned out, despite the name "Register", it's not
something that complicated to understand. 

In EMacs, register is for quick access to things. Things can be
a file location, a number, a window layout (yes, you can save window layout!)
